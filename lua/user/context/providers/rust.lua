...
function M.actions(ctx)
  if ctx.filetype == "toml" then
    ...
    return {
      {
        id = "crates.versions",
        label = "Show published versions",
        category = "Crates",
        run = crates.show_versions_popup,
      },
      { id = "crates.features", label = "Show features", category = "Crates", run = crates.show_features_popup },
      {
        ...
        category = "Crates",
        run = crates.show_dependencies_popup,
      },
      {
        id = "crates.update",
        label = "Update crate within version range",
        category = "Crates",
        run = crates.update_crate,
      },
      ...
      {
        id = "crates.upgrade_all",
        label = "Upgrade all crate ranges",
        category = "Crates",
        run = crates.upgrade_all_crates,
      },
  ...
  return {
    ...
    { id = "rust.build", label = "Choose Cargo build task", category = "Build", run = tasks.run_picker },
    {
      ...
      category = "Inspect",
      run = command "RustLsp renderDiagnostic",
    },
    {
      id = "rust.explain",
      label = "Explain current Rust error",
      category = "Inspect",
      run = command "RustLsp explainError",
    },
    ...
    {
      id = "rust.actions",
      label = "Grouped Rust code actions",
      category = "Refactor",
      run = command "RustLsp codeAction",
    },
    ...
    { id = "rust.move_up", label = "Move item up", category = "Refactor", run = command "RustLsp moveItem up" },
    {
      ...
      run = command "RustLsp rebuildProcMacros",
    },
  }
end

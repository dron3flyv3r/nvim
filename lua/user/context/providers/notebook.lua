...
local function nb(fn, arg)
  return function() require("user.integrations.notebook")[fn](arg) end
end
...
function M.actions()
  return {
    ...
    {
      id = "notebook.insert_above",
      label = "Insert cell above",
      category = "Notebook",
      run = nb("insert_cell", false),
    },
    ...
    {
      id = "notebook.output",
      label = "Show current cell output",
      category = "Output",
      priority = 110,
      run = command "MoltenShowOutput",
    },
    {
      id = "notebook.output_enter",
      label = "Enter output",
      category = "Output",
      run = command "noautocmd MoltenEnterOutput",
    },
    { id = "notebook.output_hide", label = "Hide output", category = "Output", run = command "MoltenHideOutput" },
    {
      id = "notebook.output_image",
      label = "Open output image externally",
      category = "Output",
      run = command "MoltenImagePopup",
    },
    {
      id = "notebook.output_browser",
      label = "Open HTML output in browser",
      category = "Output",
      run = command "MoltenOpenInBrowser",
    },
    {
      id = "notebook.output_delete",
      label = "Delete current cell output",
      category = "Output",
      run = command "MoltenDelete",
    },
    { id = "notebook.kernel_start", label = "Start/select kernel", category = "Kernel", run = command "MoltenInit" },
    {
      id = "notebook.kernel_register",
      label = "Register project environment as kernel",
      category = "Kernel",
      run = nb "register_kernel",
    },
    {
      id = "notebook.kernel_interrupt",
      label = "Interrupt kernel",
      category = "Kernel",
      priority = 110,
      run = command "MoltenInterrupt",
    },
    {
      id = "notebook.kernel_restart",
      label = "Restart kernel and clear output",
      category = "Kernel",
      run = command "MoltenRestart!",
    },
    { id = "notebook.kernel_stop", label = "Shut down kernel", category = "Kernel", run = command "MoltenDeinit" },
    {
      id = "notebook.import",
      label = "Import saved notebook outputs",
      category = "Notebook",
      run = command "MoltenImportOutput",
    },
    {
      id = "notebook.export",
      label = "Export outputs to notebook",
      category = "Notebook",
      run = command "MoltenExportOutput!",
    },

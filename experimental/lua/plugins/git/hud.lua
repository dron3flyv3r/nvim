...
-- Virtual text and the number column rather than a sign or a line highlight: a
-- diff background outranks both CursorLine and an extmark highlight on the same
-- line, and the sign column already carries git's own signs.
local MARK_NS = api.nvim_create_namespace "git_hud_marks"

---@alias git.hud.Took "ours"|"theirs"|"both"|"base"|"lines"|"dropped"

---@type table<git.hud.Took, string>
local TOOK = {
  ours = "took ours",
  theirs = "took theirs",
...
local function say(message) vim.notify(message, vim.log.levels.INFO, { title = "Diff review" }) end

-- A high fold level rather than `foldenable = false`, so `zM` still collapses to
-- the changed regions.
local OPEN_FOLDS = 99

local READY_TRIES = 10

local WINBAR = "%{%v:lua.require'plugins.git.hud'.winbar()%}"

---@alias git.hud.Kind "before"|"yours"|"after"|"new"|"gone"|"empty"|"ours"|"result"|"theirs"|"staged"

-- The groups are our own because Diffview remaps DiffAdd and friends per window,
-- which would repaint a label meant to say which window you are in. The three
-- merge bars borrow by pane position, so the colour on the left stays the colour
-- the left pane always has.
local BARS = {
...
---@field position string? where this file is in the changeset
...
---@type table<integer, git.hud.Pane>
local panes = {}
...
local function set_highlights()
  ...
  api.nvim_set_hl(0, "GitHudMark", { fg = hl("Special").fg or comment.fg, bold = true })
  for kind, bar in pairs(BARS) do
    local bg = hl(bar.from).bg or normal.bg
    api.nvim_set_hl(0, "GitHud" .. kind, { bg = bg, fg = normal.fg, bold = true })
    api.nvim_set_hl(0, "GitHud" .. kind .. "Dim", { bg = bg, fg = comment.fg })
    api.nvim_set_hl(0, "GitHud" .. kind .. "Alert", { bg = bg, fg = warn.fg, bold = true })
  end
end
...
---@type table<string, git.hud.Kind>
local MERGE_KINDS = { a = "ours", b = "result", c = "theirs" }

-- A rebase or cherry-pick uses the same layout with the sides swapped in
-- meaning, which is why the bars name refs rather than sides.
---@param side table?
---@return string
...
local function file_position(view)
  ...
  for index, file in ipairs(list) do
    -- A file history of one file lists that file once per commit, so the number
    -- is a position in time there rather than in a changeset.
    if file == entry then return ("%s %d/%d"):format(panel.single_file and "commit" or "file", index, #list) end
  end
end
...
-- Vim's own `]c` hunks are not enumerable, so this reproduces them. The winbar
-- asks on every redraw, hence the memo on both sides' changedtick.
---@param pane git.hud.Pane
---@return integer[][]
...
-- The same parser Diffview's own conflict actions use, so the count and the keys
-- can never disagree. A resolved region has no markers left, which is why this
-- doubles as "how much of this file is unresolved".
---@param bufnr integer?
---@return table[]
...
local function change_position(pane)
  ...
  -- For a pure insertion or deletion the count on one side is zero and the start
  -- is the line it sits behind, which is still the right line to compare.
...
-- Only meaningful in the RESULT pane: the region bounds are lines in that
-- buffer, and the other two panes sit on different line numbers wherever the
...
-- From the same hunks as the change counter, so a pending revert changes them;
-- git's own numbers describe the file on disk.
---@param pane git.hud.Pane
---@return string?
...
-- Neovim draws a window's winbar with that window current, which is how one
-- expression serves every pane.
---@return string
function M.winbar()
  local pane = panes[api.nvim_get_current_win()]
  local bar = pane and BARS[pane.kind]
  if not bar then return "" end
  local group = "GitHud" .. pane.kind

  -- Vim truncates everything after `%<` first: `keep` survives a narrow pane.
  local keep, extra = {}, {}
  ...
  elseif pane.kind == "result" then
    ...
    -- Put here by `hold` and `dress`, because working it out means reading the
    -- files this review has not opened.
    local files = usable(pane.b_buf) and vim.b[pane.b_buf].git_hud_left or 0
  ...
  if (pane.kind == "yours" or pane.kind == "result") and usable(pane.bufnr) and vim.bo[pane.bufnr].modified then
    -- Before `%<`: a narrow pane must not hide that this buffer holds edits
...
-- Diffview has no single-pane layout for a file that exists on one side only.
---@param layout table
...
local function entry_conflicts(entry)
  ...
  -- By name first: a file resolved earlier in the review is still loaded but its
  -- layout has moved on, and the copy on disk is the one with the markers in it.
...
-- Reads disk for files the review has not opened, so it is called when a file is
-- opened or finished rather than from the winbar, which runs on every redraw.
---@param view table
---@return integer count, table? first
...
-- Stopping rather than jumping is deliberate: the resolutions are on screen with
-- their marks, and this is the moment to read them.
---@param view table
...
local function hold(view, resolved)
  ...
  if usable(resolved) then vim.b[resolved].git_hud_left = left end

  if left > 0 then
    return say(("%s is resolved -- ]r and [r check it, <Tab> moves to the next of %d"):format(name, left))
  end
  say(("%s is resolved -- no conflicts left. Press q to save and stage"):format(name))
end

-- `TextChangedI` is deliberately not watched: half-typed markers come and go,
-- and a jump mid-edit would land the cursor somewhere else.
---@param view table
---@param bufnr integer
local function watch_conflicts(view, bufnr)
  ...
  api.nvim_create_autocmd("TextChanged", {
    group = api.nvim_create_augroup("git_hud_conflicts", { clear = false }),
    buffer = bufnr,
    desc = "Report when the last conflict in a file is resolved",
    callback = function()
...
function M.dress(bufnr, winid, ctx)
  ...
  if pane.kind == "result" and usable(pane.b_buf) then
    ...
    vim.b[pane.b_buf].git_hud_left = remaining(view)
  end

  -- "b" is the last pane Diffview opens, so by now both files are known and one
  -- scheduled check covers the whole file entry.
...
-- Counted across the whole changeset rather than the panes on screen: a file the
-- review has not opened is every bit as unresolved, and `git merge --continue`
-- counts it too.
---@param view table?
---@return integer
...
-- A resolved region has no markers left, so without this there is nothing to go
-- back to and nothing to check: the file just looks like a file.
---@param bufnr integer
---@param first integer
---@param count integer zero means the region was dropped
---@param took git.hud.Took
function M.mark(bufnr, first, count, took)
  ...
  -- Dropping what is left of a region you built line by line is the end of that
  -- build, not a decision to say twice.
  ...
  api.nvim_buf_set_extmark(bufnr, MARK_NS, row, 0, {
    ...
    virt_text = { { "  " .. TOOK[took], "GitHudMark" } },
    virt_text_pos = "eol",
  })
end
...
function M.next_conflict_file(view)
  ...
  -- Forwards with wrapping: "next" has to mean next, even though the file that
  -- still needs work may be one you have already walked past.
  local at = 0
  ...
  for step = 1, #files do
    ...
    if entry ~= view.cur_entry and entry_conflicts(entry) > 0 then
      say(("Moving to %s -- %d left with conflicts"):format(entry.path, remaining(view)))
...
function M.setup()
  ...
  api.nvim_create_autocmd("ColorScheme", {
    group = api.nvim_create_augroup("git_hud_highlights", { clear = true }),
    desc = "Rebuild the diff review winbar colours",

...
local function start_dir(start)
  ...
  -- Buffer names like `oil://` or a terminal are not paths.
...
function M.editor_version(root)
  local ok, lines = pcall(vim.fn.readfile, root .. "/ProjectSettings/ProjectVersion.txt", "", 4)
  if not ok then return nil end
  for _, line in ipairs(lines) do
    -- The line below it is `m_EditorVersionWithRevision`, with the hash
    -- appended; match the plain one only.
...
function M.solution(root)
  ...
  for name, kind in vim.fs.dir(root) do
    if kind == "file" and name:match "%.slnx?$" then
